...
- **Qué ve en sus círculos lo decide `CATEGORIES_LEGACY_LEAVES`** (entorno, sin publicar app). La
  app instalada los pide con `types=` y sin `section`:
  - `0` (por defecto): **los grupos**, con icono y nombre. Filtra por grupo entero; bajar a
    subcategorías llega con la app nueva.
  - `1`: las categorías de siempre, las hojas con imagen (Moda, Calzado…). Las nuevas, sin imagen,
    no entran: serían círculos vacíos.

  El mapa de la app nueva pide igual, así que sigue el mismo interruptor hasta que se pase a
  grupos.
